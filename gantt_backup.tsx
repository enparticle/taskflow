...
export default function GanttChart() {
      ...
      p.milestones?.forEach((m: any, mi: number) => {
        ...
        elements.push(
          <text key={`mlabel-${pi}-${mi}`} x={24} y={my + ROW_H / 2 + 1}
            ...
            쨌 {m.title.length > 14 ? m.title.slice(0, 14) + "?? : m.title}
          </text>
        );
  ...
  if (rows.length === 0) return (
    <div className="rounded-xl py-10 text-center"
      ...
      <p className="text-xs" style={{ color: "var(--text-3)" }}>?꾨줈?앺듃 ?놁쓬</p>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-1 mb-3">
        <span className="text-xs mr-2" style={{ color: "var(--text-3)" }}>?쒖떆 湲곌컙</span>
