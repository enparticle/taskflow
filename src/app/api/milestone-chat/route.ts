...
${categories.length > 0
  ...
  : "(미리 정해진 카테고리가 없습니다 — 대화를 시작할 때 가장 먼저 \"이 프로젝트에서 어떤 영역들의 작업이 계획되어 있나요? 편하게 나열해주세요\"라고 열린 질문으로 물어보고, 리더가 답한 카테고리들을 기준으로 그 다음부터는 위와 동일한 방식(하나씩 짚어가며 담당자·마감일·세부업무 확인)으로 진행하세요.)"}
