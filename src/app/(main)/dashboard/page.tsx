...
const STATUS_COLOR = { doing: "#60a5fa", todo: "#a1a1aa", review: "#fbbf24", blocked: "#f87171", backlog: "#52525b" };
const STATUS_LABEL = { doing: "진행 중", todo: "할 일", review: "리뷰", blocked: "Blocked", backlog: "백로그" };
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    let projQuery = supabase.from("projects")
      .select("*, milestones(id, title, status, due_date), tasks(id, status)")
    ...
    if (isAdminOrLeader) {
      ...
      const stats = (usersRaw ?? []).map((u, i) => ({
        id: u.id, name: u.name, color: COLORS[i % COLORS.length],
        doing: (allTasks ?? []).filter(t => (t.assignee_id === u.id || (t.assignee_ids ?? []).includes(u.id)) && t.status === "doing").length,
        total: (allTasks ?? []).filter(t => (t.assignee_id === u.id || (t.assignee_ids ?? []).includes(u.id))).length,
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      {!isViewer && urgentTasks.length > 0 && (
        <div className="rounded-2xl px-4 py-3 flex items-center gap-3 flex-wrap"
          ...
          <p className="text-sm font-medium" style={{ color: "#f87171" }}>긴급 업무 {urgentTasks.length}건</p>
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
            {projects.map(p => {
              ...
              const currentMilestone = (p.milestones ?? []).find(m => m.status === "in_progress");
              const nextMilestone = (p.milestones ?? []).find(m => m.status === "planned");
              ...
              return (
                <a key={p.id} href={`/projects/${p.id}`}
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = `${hc.color}66`; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = `${hc.color}33`; }}>
                  ...
                  <div className="px-5 py-3">
                    {currentMilestone && (
                      <div className="mb-1.5">
                        <p className="text-xs mb-0.5" style={{ color: "var(--text-3)" }}>진행 중 단계</p>
                    ...
                    {nextMilestone && (
                      <div>
                        <p className="text-xs mb-0.5" style={{ color: "var(--text-3)" }}>다음 단계</p>
                        <div className="flex items-center gap-1.5">
                    ...
                    {!currentMilestone && !nextMilestone && (
                      <p className="text-xs" style={{ color: "var(--text-3)" }}>단계 없음</p>
                    )}
                  </div>
                </a>
              );
        ...
        <div className="grid grid-cols-2 gap-6">
          {memberStats.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-5 rounded-full" style={{ background: "#fbbf24" }} />
                <h2 className="text-base font-bold" style={{ color: "var(--text-1)" }}>팀원 업무량</h2>
              </div>
