...
export default function CalendarPage() {
  ...
  const [form, setForm] = useState({ title: "", type: "personal", start_date: "", end_date: "", all_day: true, start_time: "", end_time: "", description: "", is_public: false, color: "" });
  ...
  const load = useCallback(async () => {
    ...
    const { data: evs } = await supabase.from("calendar_events")
      .select("*, user:users(name)").order("start_date");
    setEvents(evs ?? []);

    // 마감일 있는 업무
    const { data: ts } = await supabase.from("tasks")
      .select("id, title, status, due_date, task_type, assignee_id, assignee_ids")
  ...
  function getEventsForDay(date: Date) {
    ...
    events.forEach(ev => {
      ...
      if (isInRange(date, start, end) || isSameDay(date, start)) {
        dayEvents.push({ ...ev, _type: "event" });
      }
    });
    ...
    tasks.forEach(t => {
      ...
      if (isSameDay(date, due)) {
        dayEvents.push({ ...t, _type: "task", type: t.task_type === "meeting" ? "meeting" : "deadline" });
      }
    });
  ...
  function openNewForm(date?: string) {
    setEditEvent(null);
    setForm({ title: "", type: "personal", start_date: date ?? "", end_date: "", all_day: true, start_time: "", end_time: "", description: "", is_public: false, color: "" });
    setShowForm(true);
  }
  ...
  function openEditForm(ev: any) {
    setEditEvent(ev);
    setForm({
      title: ev.title, type: ev.type, start_date: ev.start_date, end_date: ev.end_date ?? "",
      all_day: ev.all_day ?? true, start_time: ev.start_time ?? "", end_time: ev.end_time ?? "",
      description: ev.description ?? "", is_public: ev.is_public ?? false, color: ev.color ?? "",
    });
    setShowForm(true);
  }

  const fieldStyle = { background: "var(--bg-3)", border: "1px solid var(--border-2)", color: "var(--text-1)", borderRadius: 8, padding: "6px 10px", fontSize: 13, width: "100%", outline: "none", colorScheme: "dark" as const };
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
          ...
          {days.map((d, i) => (
            <div key={i} className="px-2 py-3 text-center">
              ...
              <p className={`text-sm font-bold mt-0.5 w-7 h-7 mx-auto rounded-full flex items-center justify-center`}
                style={{
                  background: isSameDay(d, today) ? "var(--cyan)" : "transparent",
                  color: isSameDay(d, today) ? "#0D1B2E" : i === 0 ? "#f87171" : i === 6 ? "#60a5fa" : "var(--text-1)"
                }}>
                {d.getDate()}
              </p>
            </div>
        ...
        <div className="grid grid-cols-7 min-h-48">
          {days.map((d, i) => {
            ...
            return (
              <div key={i} className="p-1.5 space-y-1 cursor-pointer"
                style={{ borderRight: i < 6 ? "1px solid var(--border)" : "none", minHeight: 120, background: isSameDay(d, today) ? "rgba(34,211,238,0.03)" : "var(--bg-2)" }}
                onClick={() => openNewForm(dateStr)}>
                {dayEvs.slice(0, 4).map((ev, j) => {
                  const cfg = EVENT_TYPE_CONFIG[ev.type] ?? EVENT_TYPE_CONFIG.personal;
                  return (
                    <div key={j}
                      className="rounded px-1.5 py-0.5 text-xs truncate cursor-pointer"
                      style={{ background: `${ev.color || cfg.color}22`, color: ev.color || cfg.color, border: `1px solid ${ev.color || cfg.color}44` }}
                      onClick={e => {
                        e.stopPropagation();
                        if (ev._type === "task") setOpenDetail(ev.id);
                        else openEditForm(ev);
                      }}>
                      {ev._type === "task" ? `📌 ${ev.title}` : ev.title}
                    </div>
                  );
                })}
                {dayEvs.length > 4 && (
                  <p className="text-xs px-1" style={{ color: "var(--text-3)" }}>+{dayEvs.length - 4}개</p>
                )}
              </div>
            );
          })}
          ...
          {cells.map((d, i) => {
            ...
            return (
              <div key={i} className="p-1 cursor-pointer"
                style={{ background: isToday ? "rgba(34,211,238,0.04)" : "var(--bg-2)", borderRight: col < 6 ? "1px solid var(--border)" : "none", borderBottom: "1px solid var(--border)", minHeight: 80 }}
                onClick={() => openNewForm(dateStr)}>
                <p className={`text-xs w-5 h-5 rounded-full flex items-center justify-center mb-1 font-medium`}
                ...
                {dayEvs.slice(0, 3).map((ev, j) => {
                  ...
                  return (
                    <div key={j}
                      className="rounded px-1 py-0.5 text-xs truncate mb-0.5 cursor-pointer"
                      style={{ background: `${ev.color || cfg.color}22`, color: ev.color || cfg.color, fontSize: 10 }}
                      onClick={e => { e.stopPropagation(); if (ev._type === "task") setOpenDetail(ev.id); else openEditForm(ev); }}>
                      {ev.title}
                    </div>
                  );
                })}
                {dayEvs.length > 3 && <p className="text-xs" style={{ color: "var(--text-3)", fontSize: 9 }}>+{dayEvs.length - 3}</p>}
              </div>
            );
          })}
        </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => openNewForm(today.toISOString().slice(0, 10))}
            className="rounded-xl px-4 py-2 text-sm font-semibold ml-2"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }}
          ...
          <div className="w-full max-w-md rounded-2xl p-6 space-y-4"
            ...
            <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
              placeholder="일정 제목" style={fieldStyle} />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs mb-1 block" style={{ color: "var(--text-3)" }}>색상 (선택)</label>
                <input type="color" value={form.color || "#60a5fa"}
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-xs" style={{ color: "var(--text-2)" }}>하루 종일</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.is_public} onChange={e => setForm(f => ({ ...f, is_public: e.target.checked }))} />
                <span className="text-xs" style={{ color: "var(--text-2)" }}>팀 공개</span>
              </label>
            </div>
            ...
            {!form.all_day && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" value={form.start_time} onChange={e => setForm(f => ({ ...f, start_time: e.target.value }))} style={fieldStyle} />
                </div>
                <div>
                  <label className="text-xs mb-1 block" style={{ color: "var(--text-3)" }}>종료 시간</label>
                  <input type="time" value={form.end_time} onChange={e => setForm(f => ({ ...f, end_time: e.target.value }))} style={fieldStyle} />
                </div>
              </div>
            )}

            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              placeholder="메모 (선택)" rows={2} style={{ ...fieldStyle, resize: "none" }} />
